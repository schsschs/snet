/**
@file  win32.c
@brief SNet Win32 system specific functions
*/
#ifdef _WIN32
...
{
	return (snet_uint32)timeGetTime();
}

snet_uint32
snet_time_get(void)
{
	return (snet_uint32)timeGetTime() - timeBase;
}

void
snet_time_set(snet_uint32 newTimeBase)
{
	timeBase = (snet_uint32)timeGetTime() - newTimeBase;
}
...
snet_socket_bind(SNetSocket socket, const SNetAddress * address)
{
	...
	if (address != NULL)
	{
		sin.sin_port = SNET_HOST_TO_NET_16(address->port);
...
snet_socket_get_address(SNetSocket socket, SNetAddress * address)
{
	...
	address->port = SNET_NET_TO_HOST_16(sin.sin_port);
...
{
	return socket(PF_INET, type == SNET_SOCKET_TYPE_DATAGRAM ? SOCK_DGRAM : SOCK_STREAM, 0);
}
...
snet_socket_set_option(SNetSocket socket, SNetSocketOption option, int value)
{
	...
	case SNET_SOCKOPT_BROADCAST:
		result = setsockopt(socket, SOL_SOCKET, SO_BROADCAST, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_REUSEADDR:
		result = setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_RCVBUF:
		result = setsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_SNDBUF:
		result = setsockopt(socket, SOL_SOCKET, SO_SNDBUF, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_RCVTIMEO:
		result = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_SNDTIMEO:
		result = setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, (char *)& value, sizeof(int));
		break;

	case SNET_SOCKOPT_NODELAY:
...
snet_socket_wait(SNetSocket socket, snet_uint32 * condition, snet_uint32 timeout)
{
	...
	if (*condition & SNET_SOCKET_WAIT_SEND)
		FD_SET(socket, &writeSet);

	if (*condition & SNET_SOCKET_WAIT_RECEIVE)
	...
	*condition = SNET_SOCKET_WAIT_NONE;

	if (selectCount == 0)
		return 0;

	if (FD_ISSET(socket, &writeSet))
		* condition |= SNET_SOCKET_WAIT_SEND;

	if (FD_ISSET(socket, &readSet))
		* condition |= SNET_SOCKET_WAIT_RECEIVE;
